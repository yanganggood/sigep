...
def getpvalue(cc_dict, v_dict, M, N):
    ...
    for k in cc_dict.keys():  # cc <- cluster coefficient
        ...
        for i in range(d, min1):
            ...
            min2 = M - i
            if j <= min2 and j <= f3:
                p = getlog(f1) - getlog(i) - getlog(f1 - i) + getlog(f3) - getlog(j) - getlog(f3 - j) + getlog(f2 - f3) - getlog(M - i - j) - getlog(f2 - f3 - (M - i - j)) - fenmu
                ori += math.exp(p)
        pvalue = ori
...
def process(filename):
    ...
    for i in range(len(p_list)):
       # writeline = str(p_sorted[i]) + " : " + str(adjust_p[i]) + "\n"
        print(str(i) + ":" + str(p_sorted[i]) + " : " + str(adjust_p[i]) + "\n")
    #print(writeline)
